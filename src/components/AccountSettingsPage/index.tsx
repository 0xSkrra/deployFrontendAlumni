    ...
    <div className="flex flex-col h-screen items-center py-5 ">
      <div className="bg-rounded-md h-full flex flex-col items-center min-h-[90%] max-w-lg min-w-[75%]  rounded-lg  shadow-xl">
        <h1 className="text-5xl mt-10 mb-10 ">
          Settings for {userEdited.username}
        </h1>
        <div className=" self-center py-30">
          <form className="space-y-5" onSubmit={onSubmit}>
            <InputField
              onChange={(e) => (userEdited.status = e.target.value)}
              label={"Work Status"}
              value={userEdited.status}
            />
            <InputField
              onChange={(e) => (userEdited.funFact = e.target.value)}
              label={"Fun Fact"}
              value={userEdited.funFact}
            />
            <InputField
              onChange={(e) => (userEdited.bio = e.target.value)}
              label={"Account Bio"}
              value={userEdited.bio}
            />
            <InputField
              onChange={(e) => (userEdited.picture = e.target.value)}
              label={"Picture url"}
              value={userEdited.picture}
            />
            <button
              type="submit"
